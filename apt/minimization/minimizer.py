...
class GeneralizeToRepresentative(BaseEstimator, MetaEstimatorMixin, TransformerMixin):
    ...
    def __init__(self, estimator: Union[BaseEstimator, Model] = None, target_accuracy: Optional[float] = 0.998,
        ...
        self.accuracy_metric = accuracy_score
    ...
    def fit(self, X: Optional[DATA_PANDAS_NUMPY_TYPE] = None, y: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        ...
        if self.estimator and dataset and dataset.get_samples() is not None and dataset.get_labels() is not None:
            ...
            # check accuracy
            accuracy = self._accuracy_score(ArrayDataset(self.encoder.transform(generalized), y_test))
        ...
        for feature in ranges.keys():
            if feature not in self._generalizations['untouched']:
                ...
                if feature_ncp > 0:
                    ...
                    accuracy_gain = self._accuracy_score(ArrayDataset(self.encoder.transform(generalized), labels))\
                                    - current_accuracy
                    if accuracy_gain < 0:
                        accuracy_gain = 0
                    if accuracy_gain != 0:
    ...
    def _accuracy_score(self, score_data):
        if self.accuracy_metric is None:
            return self.estimator.score(score_data)
        else:
            return self.accuracy_metric(np.argmax(self.estimator.predict(score_data), axis=1), score_data.get_labels())
